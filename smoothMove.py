...
def smoothMove(mouseMoveX, mouseMoveY):
    win32api.mouse_event(win32con.MOUSEEVENTF_MOVE, mouseMoveX, mouseMoveY, 0, 0)


def main():
    # Window title to go after and the height of the screenshots
    videoGameWindowTitle = "Rainbow"
    ...
    # model = torch.hub.load(os.getcwd(), 'yolov5s',source='local', pretrained=True, force_reload=True)
    model = torch.hub.load('yolov5-master', source='local', model='yolov5n',
                           force_reload=True)
    model.classes = [0]
    ...
    while win32api.GetAsyncKeyState(ord(aaQuitKey)) == 0:
        ...
        if len(targets) > 0:
            ...
            if win32api.GetKeyState(0x14):
                # Todo 顺滑鼠标移动
                moveThread = threading.Thread(target=smoothMove, args=(int(mouseMove[0] * aaMovementAmp), int(mouseMove[1] * aaMovementAmp),))
                if not moveThread.is_alive():
                    moveThread.start()
